...
void main() {
  ...
  Widget _wrap(Widget child, {required ReviewCubit reviewCubit, required AuthCubit authCubit}) {
    return MaterialApp(
  ...
  testWidgets('lista vacía muestra texto de estado vacío', (tester) async {
    final mockGetPlace = MockGetReviewsFromAPlaceUseCase();
    when(() => mockGetPlace('place-x')).thenAnswer((_) async => []);
    ...
    await tester.pumpWidget(
      _wrap(
        const ReviewsListScreen(placeId: 'place-x'),
        reviewCubit: cubit,
        authCubit: auth,
      ),
    );
    ...
    expect(find.textContaining('primero'), findsOneWidget);
    await cubit.close();
  });

  testWidgets('error muestra reintentar', (tester) async {
    final mockGetPlace = MockGetReviewsFromAPlaceUseCase();
    when(() => mockGetPlace('place-e')).thenThrow(Exception('fail'));
    ...
    await tester.pumpWidget(
      _wrap(
        const ReviewsListScreen(placeId: 'place-e'),
        reviewCubit: cubit,
        authCubit: auth,
      ),
    );
